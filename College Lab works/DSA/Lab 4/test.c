...
void node_swap(struct Employee *left, struct Employee *right)
{
  struct Employee *tmp;
...
void cluster() {
    struct Employee* left=head,*right=head->next->next;
    while(left && right->next!=NULL){
        while(right){
            if(strcmp(left->Emp_Id, right->Emp_Id)==0){
                while(strcmp(left->Emp_Dept,right->Emp_Dept)==0){
                    left=left->next;
                }
                swap(left,right);
            }
            struct Employee* temp=left->next;
